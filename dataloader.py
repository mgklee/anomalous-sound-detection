...
class test_dataset(torch.utils.data.Dataset):
    def __init__(self, root_path, test_name, name_list):
        ...
        self.target_idx = name_list.index(test_name)
    ...
    def __getitem__(self, idx):
        return self.y_list[idx], self.y_spec_list[idx], self.labels[idx], self.target_idx, self.y_true[idx]
...
class train_dataset(torch.utils.data.Dataset):
    def __init__(self, root_path, name_list):
        ...
        for i, files in enumerate(files_list):
            ...
            for file_name in files:
                for idx, label_idx in enumerate(label_list):
                    if label_idx in file_name:
                        self.id_labels.append(idx + maximum)
                        self.type_labels.append(i)
            maximum = max(self.id_labels) + 1

        self.unrolled_files_list = list(itertools.chain.from_iterable(files_list))
        self.id_labels = torch.LongTensor(self.id_labels)
        self.type_labels = torch.LongTensor(self.type_labels)
    ...
    def __getitem__(self, idx):
        return self.y_list[idx], self.y_spec_list[idx], self.id_labels[idx], self.type_labels[idx]
